...
def get_ego_traj(imu_poses, frame, pref, futf, inverse=False, only_fut=False):
    ...
    print("imu_poses",imu_poses)
    #imu_poses=imu_poses[0]
    #frame=frame-1

    if frame>3 and num_frames>2:
    #assert frame >= 0 and frame <= num_frames - 1, 'error'
        if inverse:             # pre and fut are inverse, i.e., inverse ego motion compensation
            start = frame+pref-1 #min(frame+pref-1, num_frames-1)
            end   = frame-futf-1 #max(frame-futf-1, -1)
            ...
            end   = frame+futf+1 #min(frame+futf+1, num_frames)
        ...
        cur_world_xyz = imu_poses[2]   #frame                     # 4 x 4, frame = 9
        ...
        cur_world_rot = imu_poses[2, :3, :3]                # 3 x 3, frame = 9
