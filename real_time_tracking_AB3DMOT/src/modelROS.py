...
from AB3DMOT.AB3DMOT_libs.dist_metrics import iou
from AB3DMOT.AB3DMOT_libs.vis import vis_obj

from kalman_filter import KF, STATE_X, STATE_Y, STATE_Z, STATE_THETA, STATE_L, STATE_W, STATE_H, STATE_DX, STATE_DY, STATE_DZ
...
class AB3DMOT():
	def __init__(self,  ID_init=0, detections_topic=None):
		...
		self.detections_sub = rospy.Subscriber(self.detections_topic, BoundingBoxArray, self.detections_callback)
		...
		# ds.registerCallback(self.detections_callback)
		self.marker_pub = rospy.Publisher( detections_topic+'/tracked/visualization', MarkerArray)
		self.bbox_publish = rospy.Publisher(detections_topic+"/tracked", BoundingBoxArray, queue_size=10)
		...
		cfg=namedtuple('cfg',
					   ['description',
						...
						'vis',
						'affi_pro',
						'confidence_weight_on_tracklet_score'])
		...
		cfg.vis=False
		cfg.affi_pro=True
		cfg.confidence_weight_on_tracklet_score=0.2

		# config
		self.cat = "Car"
		self.ego_com = cfg.ego_com 		# ego motion compensation'
		self.affi_process = cfg.affi_pro	# post-processing affinity
		self.confidence_weight_on_tracklet_score = cfg.confidence_weight_on_tracklet_score
	...
	def transform_object_list(self, objects: BoundingBoxArray, transform_stamped):
		...
		rot = R.from_quat([transform_stamped.transform.rotation.x, transform_stamped.transform.rotation.y, transform_stamped.transform.rotation.z, transform_stamped.transform.rotation.w])
		...
		for obj in objects.boxes:
			...
			rot = R.from_quat([obj.pose.orientation.x, obj.pose.orientation.y, obj.pose.orientation.z, obj.pose.orientation.w])
			...
			rot = R.from_matrix(transformed_pose[0:3, 0:3])
			quat = rot.as_quat()
	...
	def get_param(self, cfg, cat):
		...
		if cfg.dataset == 'KITTI':
			if cfg.det_name == 'pvrcnn':				# tuned for PV-RCNN detections
				...
				if cat == 'Car': 			algm, metric, thres, min_hits, max_age = 'hungar', 'giou_3d', -0.2, 3, 5
	...
	def detections_callback(self, in_detections:BoundingBoxArray):
		...
		lidar2map = self.get_transform("map", in_detections.header.frame_id, in_detections.header.stamp)
		if lidar2map is None:
			return

		transformed_objects = self.transform_object_list(in_detections, lidar2map)

		dets = []
		label = []
		scores = []

		for i, obj in enumerate(transformed_objects.boxes):
			# dets format : hwlxyzo + class=
			_, _, yaw = euler_from_quaternion(obj.pose.orientation.x, obj.pose.orientation.y, obj.pose.orientation.z, obj.pose.orientation.w)
			box = [obj.dimensions.z, obj.dimensions.y, obj.dimensions.x, obj.pose.position.x, obj.pose.position.y, obj.pose.position.z, yaw]
			dets.append(box)
		...
		for i, trk in enumerate(trk_result):
			...
			if trk[9]>0.01:
				...
				marker.header.stamp = in_detections.header.stamp
				marker.type = marker.TEXT_VIEW_FACING
				marker.id = int(trk[7])
				marker.text = f"{int(trk[7])} {cats[int(trk[8])]} {trk[10]:.1f} m/s"
				...
				bbox.header.frame_id = "map" #.seq = int(trk[7])
				bbox.header.stamp = in_detections.header.stamp
	...
	def prediction(self):
		...
		for t in range(len(self.trackers)):
			...
			# kf_tmp.kf.F[STATE_X, STATE_DX] = 0.1
			kf_tmp.kf.predict()
	...
	def update(self, matched, unmatched_trks, dets, info):
		...
		for t, trk in enumerate(self.trackers):
			if t not in unmatched_trks:
				...
				trk.kf.x[2] = dets[d[0]].z # update z
				trk.info = info[d, :][0]
	...
	def birth(self, dets, info, unmatched_dets):
		...
		for i in unmatched_dets:        			# a scalar of index
			trk = KF(Box3D.bbox2array(dets[i]), info[i, :], self.ID_count[0], self.frame_count)
			self.trackers.append(trk)
			new_id_list.append(trk.id)
			self.ID_count[0] += 1

		return new_id_list
	# Calculate score for each tracklet
	# Score = (hits / (frame_count - starting_frame + 1) + // ratio of hits to the total lifetime of the tracklet
	#         - time_since_update / max_age)  // penalize the tracklet if it does not update for a long time
	#         * confidence  // multiply by the confidence of the detection
	def life_cycle(self):
		...
		for trk in self.trackers:
			...
			track_score = trk.hits / (self.frame_count - trk.starting_frame + 1) -\
						  trk.time_since_update / self.max_age

			# penalize the trackless if it does not update for a long time, but not too much
			trk.score = track_score - track_score * self.confidence_weight_on_tracklet_score * trk.info[1]
			track_bboxes.append(Box3D.array2bbox(trk.kf.x[:7].reshape((7, ))))

		for trk1_idx in range(len(self.trackers)):
			trk1 = self.trackers[trk1_idx]
			trk1_box = track_bboxes[trk1_idx]
			trk1_max_edge = max(trk1_box.l, trk1_box.w)

			# if trk1.hits < self.min_hits:
			# 	continue

			# bbox location self
			for trk2_idx in range(trk1_idx+1, len(self.trackers)):
				trk2 = self.trackers[trk2_idx]

				# if trk2.hits < self.min_hits:
				# 	continue

				trk2_box = track_bboxes[trk2_idx]

				trk2_max_edge = max(trk2_box.l, trk2_box.w)

				# If objects are close enough, go on with calculating the IoU
				dist_for_collision = max(trk1_max_edge, trk2_max_edge)
				dist_squared_2d = (trk1_box.x - trk2_box.x) ** 2 + (trk1_box.y - trk2_box.y) ** 2

				# If the distance between the centers of the two objects is less than the sum of their lengths, then they are close enough to be considered for collision
				if dist_squared_2d > dist_for_collision ** 2:
					continue

				overlap = iou(trk1_box, trk2_box, 'ioma_2d')
				if overlap > 0.5:
					if trk1.score < trk2.score:
						trk1.score = -1
					else:
						trk2.score = -1
		updated_tracks = []
		for trk in self.trackers:
			if trk.score != -1:
				updated_tracks.append(trk)	# only keep the tracklet with positive score
	...
	def output(self):
		...
		for trk in reversed(self.trackers):
			...
			vel = np.linalg.norm(trk.get_velocity())
			if ((trk.time_since_update < self.max_age) and (trk.hits >= self.min_hits or self.frame_count <= self.min_hits)):
				results.append(np.concatenate((d, [trk.id], trk.info, [vel])).reshape(1, -1))
			num_trks -= 1
...
if __name__ == '__main__':
	...
	objects_topic="/carla/ego_vehicle/lidar/measurements"
	# model = init_model(config_file, checkpoint_file, device)
	ID_start=1
	AB3DMOT( ID_init=ID_start, detections_topic=objects_topic)